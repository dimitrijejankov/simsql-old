...
public class FoulaOptimizer implements Optimizer <SimSQLCompiledQuery, SimSQLOptimizedQuery> {
	...
	*/
    ...
    public void inferNextRandomTable(Operator operator, String table, HashMap<String, PreviousTable> previousResultTableMap, ExampleRuntimeParameter params) 
    {
	...
	if (DEBUG) {
	    System.out.println("It starts to infer Next RandomTable: " + table);
	}
		
	table = reverseTableName(table);
	String tempQueryFile = table + "_plan_next.pl";
	try
	    {
		BufferedWriter wri = new BufferedWriter(new FileWriter(tempQueryFile, false));
	
		ArrayList<String> sinkTableList = new ArrayList<String>();
		sinkTableList.add(table);
		...
		wri.write("parent(planRoot, " + frameOutput.getNodeName() + ").\r\n");
		...
		DataOutputStream wro = new DataOutputStream(
							    p2.getOutputStream());
		wro.writeBytes("consult(['control_transformations','graphoperations','lib','transformations','plancost','optimizer']).\n");
		wro.writeBytes("consult('" + tempQueryFile + "').\n");
		wro.writeBytes("optimize(optimize, " + OPT_MAX_ITERATIONS + ", " + OPT_MAX_PLANS  + ",A,B,C,D, 'Q_opt.pl').\n");
		wro.flush();
		...
		// check if the optimizer produced the desired output
		if (!(new File(tempQueryFile)).exists()) {
		    throw new Exception("Could not execute the optimizer!");
		}
		else
		    {
			String lines[] = FileOperation.getFile_content("Q_info.pl");
			for(int i = 0; i < lines.length; i++)
			    {
				PreviousTable temp = this.parseResultTableMetaDataForEstimator(lines[i], operator);
				previousResultTableMap.put(temp.getTableName(), temp);
			    }
				
			new File(tempQueryFile).delete();
			new File("Q_info.pl").delete();
			new File("Q_opt.pl").delete();
		    }
	    ...
	    {
		e.printStackTrace();
		throw new RuntimeException("exception during inferNextRandomTable");
	    }
		
	if (DEBUG) {
	    System.out.println("It ends to infer Next RandomTable: " + table);
	}
    }
