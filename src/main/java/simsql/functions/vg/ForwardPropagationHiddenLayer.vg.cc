...
class ForwardPropagationHiddenLayer : public VGFunction {
...
public:
  ...
  int outputVals(RecordOut &out) {
    ...
    for(int i = 0; i < product->size1; ++i) {
        for(int j = 0; j < product->size2; ++j) {
            double tmp = gsl_matrix_get(product, i, j) + gsl_matrix_get(b, 0, j);
            out.o->value[i * product->size2 + j] = tmp > 0 ? tmp : 0;
        }
    }
  ...
  const char *getName() {
    return "ForwardPropagationHiddenLayer";
  }
};


VGFunction *create() {
  return(new ForwardPropagationHiddenLayer());
}

void destroy(VGFunction *vgFunction) {
  delete (ForwardPropagationHiddenLayer *)vgFunction;
}