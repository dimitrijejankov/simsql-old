...
void handler (const char * reason,
  ...
  runningError = 1;
  fprintf(stderr, "Some error happened in Uniform_WordTopic_Collapsed...\n");
}


class Uniform_WordTopic_Collapsed: public VGFunction {
private:
...
public:
  ...
  Uniform_WordTopic_Collapsed() {
    ...
    outputCounts = (unsigned int **)malloc(DICTIONARY * sizeof(unsigned int *));
    for (int i=0;i<DICTIONARY;i++) {
      outputCounts[i] = (unsigned int *)malloc(TOPICS * sizeof(unsigned int));
    }
  ...
  const char *getName() {
    return "Uniform_WordTopic_Collapsed";
  }
};

/** External creation/destruction methods -- DO NOT MODIFY */
VGFunction *create() {
  return(new Uniform_WordTopic_Collapsed());
}

void destroy(VGFunction *vgFunction) {
  delete (Uniform_WordTopic_Collapsed *)vgFunction;
}
