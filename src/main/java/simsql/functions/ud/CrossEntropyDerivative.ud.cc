...
class CrossEntropyDerivative : public UDFunction {
...
public:
    ...
    */
    ...
    gsl_matrix* executeMatrix(RecordIn* in) {
        ...
        for (size_t i = 0; i < output->size1; i++) {
            for (size_t j = 0; j < output->size2; j++) {
                if ((size_t) *(in->columnID) == (((size_t)in->category->data[i]) / output->size2)
                    && j == ((size_t)in->category->data[i]) % output->size2) {
                    gsl_matrix_set(output, i, j, gsl_matrix_get(in->m->matrix, i, j) - 1);
                } else {
                    gsl_matrix_set(output, i, j, gsl_matrix_get(in->m->matrix, i, j));
                }
            }
        }
     ...
     */
    std::string getName() {
        return std::string("cross_entropy_derivative");
    }
...
UDFunction *create() {
  return(new CrossEntropyDerivative());
}

void destroy(UDFunction *udFunction) {
  delete (CrossEntropyDerivative *)udFunction;
}