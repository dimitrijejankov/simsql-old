...
class MatrixMatrixMultiply : public UDFunction {
...
public:
    ...
    */
     ...
     */
    std::string getName() {
        return std::string("matrix_matrix_multiply");
    }
...
UDFunction *create() {
  return(new MatrixMatrixMultiply());
}

void destroy(UDFunction *udFunction) {
  delete (MatrixMatrixMultiply *)udFunction;
}