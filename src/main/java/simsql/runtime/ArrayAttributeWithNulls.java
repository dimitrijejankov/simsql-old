...
public class ArrayAttributeWithNulls implements Attribute {
    ...
    public int writeSelfToStream(DataOutputStream writeToMe) throws IOException {
        int returnVal = bitstringMachine.writeToStream(theseOnesAreNull, writeToMe);
        return returnVal + attributeMachine.writeToStream(myAttribute, writeToMe);
    }
    ...
    public byte[] getValue(int whichMCIter) {
        if (!theseOnesAreNull.getValue(whichMCIter)) {
            return myAttribute.getValue(whichMCIter);
        } else {
            throw new RuntimeException("you tried to get a value that was NULL");
        }
    }

    public byte[] getValue(int whichMCIter, AttributeType castTo) {
        if (!theseOnesAreNull.getValue(whichMCIter)) {
            return myAttribute.getValue(whichMCIter, castTo);
        } else {
            throw new RuntimeException("you tried to get a value that was NULL");
        }
    }
