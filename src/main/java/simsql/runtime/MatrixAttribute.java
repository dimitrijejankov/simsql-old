...
public class MatrixAttribute implements Attribute {
  ...
  */
    ...
    // the matrix in injectValue will always be row-based
    ...
    public void injectIntoBuffer(int whichMC, AttributeType castTo, LargeByteBuffer buffer) {
        if (castTo.getTypeCode() == getType(whichMC).getTypeCode()) {
            ...
            if (ifRow) {
                buffer.putDouble(myVals.length);
                buffer.putDouble(secondDimension);
                for (double[] myVal : myVals) {
                    if (myVal != null) {
                        for (int j = 0; j < secondDimension; j++) {
                            buffer.putDouble(myVal[j]);
                        }
                    } else {
                        for (int j = 0; j < secondDimension; j++) {
    ...
    public long readSelfFromStream(DataInputStream readFromMe) throws IOException {
        long returnVal;
        int numNonZeros, len, currentIndex = 0;
        ArrayList<Integer> nonZeros = new ArrayList<Integer>();
