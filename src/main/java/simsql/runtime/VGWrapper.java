...
class VGWrapper {
    ...
    private LargeByteBuffer dataBuffIn;
    private LargeByteBuffer dataBuffOut;

    private long dataStartPosition = 0;  // for the cross products.
     ...
     */
    public VGWrapper(long bufferSize, VGFunction vgf, int numMC, OutputAdapter writer, ObjectWithPipeNetwork pipe) {
        ...
        ByteBuffer pbo = ByteBuffer.allocateDirect((int) bufferSize / 10);
        ...
        // and position buffer.
        ByteBuffer pbi = ByteBuffer.allocateDirect((int) bufferSize / 10);
        pbi.order(ByteOrder.nativeOrder());
        posBuffIn = pbi.asLongBuffer();

        // declare the tuple buffer.
        ByteBuffer tbi = ByteBuffer.allocateDirect((int) bufferSize / 10);
    ...
    public void takeInnerRec(VGRecord innerRec) {
        ...
        if (numInnerRecs >= innerRecs.length) {
            ...
            System.arraycopy(innerRecs, 0, newInnerRecs, 0, numInnerRecs);
...
class VGIntermediateValue {
    ...
    public void readFrom(VGRecord inputRec, AttributeType[] inTypes, int numMC, LargeByteBuffer dataBuffIn, LongBuffer posBuffIn) {
        ...
        for (int i = 0; i < numVals; i++) {
            ...
            for (int j = 0; j < inTypes.length; j++) {
                ...
                atts[j].injectIntoBuffer(i, inTypes[j], dataBuffIn);
            }
        }
    }
